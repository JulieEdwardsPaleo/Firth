...
ax.set_ylabel('Window length (days)',fontsize=10)
...
ax_top.set_xlabel('Month',fontsize=10)
fig.text(-0.02,1,'b)')
...
###############################################
# Load the dataset
file_path = '/Users/julieedwards/Documents/Projects/MANCHA/Climate/monthly/iberkeley_tavg_-141.05E_68.67N_n.nc'
dataset = nc.Dataset(file_path)
time = dataset.variables['time'][:]
temp = dataset.variables['temperature'][:]
dates = pd.date_range(start='1828-01-15', periods=len(time), freq='M')
data = pd.DataFrame({'time': dates, 'tmp': temp})

###### ERA5 ###################
file_path='/Users/julieedwards/Documents/Projects/MANCHA/Climate/monthly/iera5_t2m_-141.05E_68.67N_1950-2022_n.nc'
dataset = nc.Dataset(file_path)
time = dataset.variables['time'][:]
...
axs[0].set_title('1-Month',fontsize=10)
axs[0].set_xlabel('End Month',fontsize=10)
axs[0].set_ylabel('R',fontsize=10,rotation=0)
axs[0].set_ylim(ylim)
...
axs[1].set_title('2-Month',fontsize=10)
axs[1].set_xlabel('End Month',fontsize=10)
axs[1].set_ylim(ylim)
axs[1].set_xticks([1,3,5,7,9,11])
axs[1].set_xticklabels(months_labels,fontsize=10)
...
axs[2].set_title('3-Month',fontsize=10)
axs[2].set_xlabel('End Month',fontsize=10)
axs[2].set_ylim(ylim)
axs[2].set_xticks([1,3,5,7,9,11])
axs[2].set_xticklabels(months_labels,fontsize=10)
...
axs[3].set_title('4-Month',fontsize=10)
axs[3].set_xlabel('End Month',fontsize=10)
axs[3].set_ylim(ylim)
axs[3].set_xticks([1,3,5,7,9,11])
axs[3].set_xticklabels(months_labels,fontsize=10)
axs[3].grid(True, which='both', axis='both', linestyle='--', linewidth=0.7, zorder=0)
axs[3].set_axisbelow(True)
blue_patch = plt.Line2D([0], [0], color='blue', lw=4, label='p < 0.01')
lightblue_patch = plt.Line2D([0], [0], color='lightblue', lw=4, label='p ≥ 0.01')
legend=axs[0].legend(handles=[blue_patch, lightblue_patch], loc='upper left',fontsize=8,handlelength=1)
...
file_path = '/Users/julieedwards/Documents/Projects/MANCHA/Climate/annual_average_temperature.nc'
...
# Assuming lat_subset, lon_subset, and correlation_subset are already defined

# Define discrete bins for the correlation values
bins = np.linspace(-0.8, 0.8, 17)  # 10 bins from -1 to 1
fig = plt.figure(figsize=(7, 5))
ax = plt.axes(projection=ccrs.PlateCarree())
ax.set_extent([-180, -50, 40, 90], crs=ccrs.PlateCarree())
ax.coastlines()
lon_grid, lat_grid = np.meshgrid(lon_subset, lat_subset)

# Plot the correlation map with discrete bins
correlation_plot = ax.contourf(lon_grid, lat_grid, correlation_subset, bins, cmap='coolwarm', extend='both', transform=ccrs.PlateCarree())
stipple_mask = p_value_subset > 0.05
ax.scatter(lon_grid[stipple_mask], lat_grid[stipple_mask], color='k', s=1, transform=ccrs.PlateCarree())
cbar = fig.colorbar(correlation_plot, ax=ax, orientation='vertical', fraction=0.0185,
                    pad=0.01,ticks=[-0.8,-0.4,0,0.4,0.8],aspect=20,location='right')
cbar.set_label('R', rotation=0, labelpad=5)
lat_point = 68.67
lon_point = -141.05
ax.plot(lon_point, lat_point, 'o', markersize=5, color='k',transform=ccrs.PlateCarree())
# Add a label for the point
ax.text(lon_point - 3, lat_point+2, 'Firth River', transform=ccrs.PlateCarree())
ax.add_feature(cfeature.BORDERS)
ax.add_feature(cfeature.LAND)
ax.add_feature(cfeature.OCEAN)
plt.title('ERA5 195 to 223 DOY mean',fontsize=10,y=-.11)
fig.text(.128,.67,'c)')
#plt.tight_layout()
plt.show()
...
#plt.subplots_adjust(left=0.05, right=0.95, top=1, bottom=0, hspace=.5)
...
#plt.subplots_adjust(left=0.05, right=0.95, top=0.95, bottom=0.05, hspace=.5)
...
plt.savefig('climatepanel_ERA5_10.eps', format='eps',bbox_inches='tight')
plt.show()
