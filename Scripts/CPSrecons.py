...
r10=[H10['pbw10'],H30['pbw10'],normalR['pbw10'],L5['pbw10'],L10['pbw10'],L20['pbw10']]
r20=[H10['pbw20'],H30['pbw20'],normalR['pbw20'],L5['pbw20'],L10['pbw20'],L20['pbw20']]
r40=[H10['pbw40'],H30['pbw40'],normalR['pbw40'], L5['pbw40'],L10['pbw40'],L20['pbw40']]
r80=[H10['pbw80'],H30['pbw80'],normalR['pbw80'],L5['pbw80'],L10['pbw80'],L20['pbw80']]
...
if abs(correlation) > sig_level:
    print("Significant correlation:", correlation)
else:
    print("Non-significant correlation:", correlation)

fig=plt.subplots(figsize=(4,3))
plt.plot(r10,'-o',label='pbw10',color='#D75E9B')
plt.plot(r20,'-o',label='pbw20',color='#A21C57')
plt.plot(r80,'-o',label='pbw80',color='#660C23')
plt.ylabel('R',fontsize=9)
plt.xlabel('High-pass filter (n years)  Low-pass filter (n years)',fontsize=9)
plt.grid(linestyle='--',alpha=0.5)
plt.xticks([0,1,2,3,4],labels=['10','30','None','5','20'],fontsize=9)
plt.tight_layout()
...
mrw=np.mean(rw,axis=1)
mrw=mrw.loc[1960:2021]
sfrcs_filtered=recons_df.loc[1960:2021][['pbw10','pbw20','pbw40','pbw80','bw100']]
filtered_indices = mrw.index
sfrcs_filtered = sfrcs_filtered.loc[filtered_indices]
mk_results = {}
for column in sfrcs_filtered.columns:
    result = mk.original_test(sfrcs_filtered[column])
    mk_results[column] = result.slope
mk_df = pd.DataFrame(list(mk_results.items()), columns=['Column', 'Mann-Kendall_Slope_Coefficient'])
norm = plt.Normalize(mk_df['Mann-Kendall_Slope_Coefficient'].min(), mk_df['Mann-Kendall_Slope_Coefficient'].max())
colors = plt.cm.Greys(norm(mk_df['Mann-Kendall_Slope_Coefficient']))

plt.figure(figsize=(3, 4))
plt.axvline(x=0, color='grey', linestyle='--')
plt.scatter([0] * len(mk_df), mk_df['Mann-Kendall_Slope_Coefficient'], c=colors, cmap='Greys', s=100, edgecolor='black', linewidth=1.5)
plt.title('1800-2021 slopes [MRW<300um]')
for i, row in mk_df.iterrows():
    plt.annotate(row['Column'], (0, row['Mann-Kendall_Slope_Coefficient']), textcoords="offset points", xytext=(5,0), ha='left')
...
plt.tight_layout()
#plt.savefig('pbwmkindustrial300.eps',format='eps',bbox_inches='tight')
plt.show()
...
models=models.loc[1150:2021]-273.15
percentiles = models.apply(lambda x: pd.Series({
    '5th': x.quantile(0.05),
...
mk_df.index=mk_df['Column']
slope=mk_df.loc['pbw10']['Mann-Kendall_Slope_Coefficient']
intercept = recons_df.loc[1800:2021]['pbw10'].mean() - slope * np.array(recons_df.loc[1800:2021].index).mean()
...
plt.ylim(0.75,0.9)
plt.xticks([0,1,2,3,4],labels=subrecons_df.columns,fontsize=9)
plt.grid()
plt.title('pq corr with 2013MXD')
plt.ylabel('R')
plt.savefig('2013corrpq.eps',format='eps',bbox_inches='tight')
...
ax1.text(2000,18,'10 $\mu$m R$^2$=0.64',color='#D75E9B',fontsize=9)
ax1.text(2000,16,'20 $\mu$m R$^2$=0.61',color='#A21C57',fontsize=9)
ax1.text(2000,14,'80 $\mu$m R$^2$=0.48',color='#660C23',fontsize=9)
...
ax3.set_xticks([0,1,2,3,4,5],labels=['H10','H30',' NA','L5','L10','L20'],fontsize=9)
...
plt.savefig('Reconstructionpanels_filter.eps', format='eps',bbox_inches='tight')
plt.show()
...
ax1.plot(sea_10[['CI_5th','CI_95th']],linestyle='--',color='#D75E9B',linewidth=1)
ax1.plot(sea_20[['CI_5th','CI_95th']],linestyle='--',color='#A21C57',linewidth=1)
ax1.plot(sea_80[['CI_5th','CI_95th']],linestyle='--',color='#660C23',linewidth=1)
...
for i, col in enumerate(group2):
    nop=ax2.plot(sea_bw.index, sea_bw[col], label=f'{col}', linestyle=line_styles_group2[i], 
             color=colors_group2[i],linewidth=width_group2[i])
#plt.plot(sea_bw.index, sea_bw, marker='o')
ax2.plot(seaq_10[['CI_5th','CI_95th']],linestyle='--',color='#D75E9B',linewidth=1)
ax2.plot(seaq_20[['CI_5th','CI_95th']],linestyle='--',color='#A21C57',linewidth=1)
ax2.plot(seaq_80[['CI_5th','CI_95th']],linestyle='--',color='#660C23',linewidth=1)
...
plt.savefig('SEAsigl2015.eps',format='eps',bbox_inches='tight')
plt.show()


forcings=pd.read_csv('/Users/julieedwards/Documents/Projects/MANCHA/Climate/Burkes_tabula-toohey_supplemental_extracted.csv',header=0,index_col=None)

forcings=pd.DataFrame(None)
events=[1258, 1458, 1815, 1230, 1783, 1809, 1641, 1601, 1171, 1695, 1286, 1345, 1276, 1836, 1991, 1832, 1453, 1191, 1595, 1182, 1884]
forcings['Year']=events

forcings['MaxResponse10']=None
forcings['MaxResponse80']=None
forcings['maxloc10']=None
forcings['maxloc80']=None

df=recons_df['pbw10']
for event in forcings['Year']:
    backstart,backend=event-3,event-1
    mean=df.loc[backstart:backend].mean()
    response=df.loc[event]-mean
    maxtempdelta=response
    maxdeltaloc = event

    forcings.loc[forcings['Year'] == event, 'MaxResponse10'] = response

df=recons_df['pbw80']
for event in events:
    backstart,backend=event-3,event-1
    mean=df.loc[backstart:backend].mean()
    response=df.loc[event:event+2]-mean
    maxtempdelta=np.min(response)
    maxdeltaloc = event

    forcings.loc[forcings['Year'] == event, 'MaxResponse80'] = maxtempdelta

from sklearn.linear_model import LinearRegression

X = forcings['VSSI_V3'] 
y = forcings['MaxResponse80']
X = X.dropna().values.reshape(-1, 1)
y = y.dropna().values
model = LinearRegression()
model.fit(X, y)
line_fit80 = model.predict(X)
X = forcings['VSSI_V3'] 
y = forcings['MaxResponse10']
X = X.dropna().values.reshape(-1, 1)
y = y.dropna().values
model = LinearRegression()
model.fit(X, y)
line_fit10 = model.predict(X)


plt.plot(forcings['VSSI_V3'],forcings['MaxResponse10'],'ko')
plt.plot(forcings['VSSI_V3'],forcings['MaxResponse80'],'ro')
plt.plot(X, line_fit10, color='k', label='pbw10line')
plt.plot(X, line_fit80, color='red', label='pbw80line')
plt.xlabel('VSSI')
plt.ylabel('Max Temp Delta')
plt.legend()
...
rw=rw.drop(columns=['YEAR'])
mrw=np.mean(rw,axis=1)
mrw_filtered = mrw.loc[1150:2021]

forcings.index=forcings['Year']
forcings['MRW']=mrw_filtered.loc[events]
import matplotlib.colors as mcolors

num_bins = 5
bins = np.linspace(forcings['MRW'].min(), forcings['MRW'].max(), num_bins + 1)

# Discretize the MRW data into bins
forcings_binned = np.digitize(forcings['MRW'], bins) - 1

# Define a colormap and normalize based on the bins
cmap = plt.get_cmap('viridis', num_bins)
norm = mcolors.BoundaryNorm(bins, cmap.N)

x=forcings['MaxResponse10']
y=forcings['MaxResponse80']
fig=plt.subplots(figsize=(6,5))
plt.grid(linewidth=0.5,linestyle='--')
scatter=plt.scatter(forcings['MaxResponse10'],forcings['MaxResponse80'],label=forcings['Year'],c=forcings['MRW'], cmap=cmap, norm=norm,s=50)
for i in events:
    plt.annotate(str(forcings['Year'][i]), (x[i], y[i]), textcoords="offset points", xytext=(0,5), ha='center',fontsize=5)
plt.xlabel('pbw10 Year 0 temperature response')
plt.ylabel('pbw80 Year 0 temperature response')
plt.xlim(-8,6)
plt.ylim(-8,6)
plt.plot([-12, 6], [-12, 6], color='k', label='1:1 line',linewidth=1)
